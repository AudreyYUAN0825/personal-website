...
/* ── QR Popup ─────────────────────────────────────────────────────────────── */
function QrPopup({ src, label, color, onClose }: { src: string; label: string; color: string; onClose: () => void }) {
  return (
    <div
    ...
    >
      <div
        className="ui-card relative w-full max-w-[260px] p-6"
...
/* ── Social button ─────────────────────────────────────────────────────────── */
function SocialBtn({
  href, onClick, color, icon, label, badge,
}: {
  href?: string; onClick?: () => void; color: string;
  icon: React.ReactNode; label: string; badge?: string;
}) {
  const cls = "group flex items-center gap-2 rounded-full border border-line bg-white/58 px-4 py-2.5 text-[12.5px] font-medium text-ink/55 shadow-soft transition-all duration-280 hover:bg-white/85 hover:text-ink hover:shadow-hover";
  const inner = (
    <>
    ...
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-12">
          ...
          <div className="ui-card p-6 md:p-8">
              ...
              <div className="flex flex-col gap-2">
                {contact.emails?.map((email) => (
                  <a
                    key={email}
                    href={`mailto:${email}`}
                    className="flex items-center gap-2.5 text-[16px] font-medium text-ink/65 transition hover:text-accent"
                  >
                    <EmailIcon />
                    {email}
                  </a>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {contact.linkedin && (
                  <SocialBtn href={contact.linkedin} color="#0077B5" icon={<LinkedInIcon />} label="LinkedIn" />
                )}
                {contact.xiaohongshu && (
                  <SocialBtn href={contact.xiaohongshu} color="#FF2442" icon={<XhsIcon />} label="小红书" badge="7.4万" />
                )}
                {contact.wechat_qr && (
                  <SocialBtn onClick={() => setQrOpen("wechat")} color="#07C160" icon={<WeChatIcon />} label="微信" />
                )}
                {contact.whatsapp_qr && (
                  <SocialBtn onClick={() => setQrOpen("whatsapp")} color="#25D366" icon={<WhatsAppIcon />} label="WhatsApp" />
                )}
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4 lg:col-span-5">
          ...
          <div className="ui-card flex items-start gap-3 p-5">
            <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-accent/20 bg-accent/[0.07]">
              <span className="h-2.5 w-2.5 animate-pulse rounded-full bg-accent" />
            </div>
            <div>
              <p className="text-[13px] font-semibold leading-snug text-ink/80">{availabilityNote[locale]}</p>
              <p className="mt-1.5 text-[12px] leading-relaxed text-ink/48">{availabilitySub[locale]}</p>
            </div>
          </div>
      ...
      {qrOpen === "wechat" && contact.wechat_qr && (
        <QrPopup src={contact.wechat_qr} label="微信 WeChat" color="#07C160" onClose={() => setQrOpen(null)} />
      )}
      {qrOpen === "whatsapp" && contact.whatsapp_qr && (
        <QrPopup src={contact.whatsapp_qr} label="WhatsApp" color="#25D366" onClose={() => setQrOpen(null)} />
      )}
    </>
  );
}
